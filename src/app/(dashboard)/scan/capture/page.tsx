...
export default function CapturePage() {
  ...
  useEffect(() => {
    if (!config) { router.replace("/scan"); return; }
    ...
    try {
      ...
      const { data: scan, error: scanErr } = await db
        ...
        .insert({
          ...
          left_height_mm:   cmp.leftMm,
          right_height_mm:  cmp.rightMm,
          left_width_mm:    null,
          right_width_mm:   null,
      ...
      if (result.annotatedBlob) {
        ...
        if (!storageErr) {
          ...
          await db.from("scan_images").insert({
            scan_id: scan.id, side: "pair", storage_path: filename, public_url: urlData.publicUrl,
          });
        }
      }
      setSaveStatus("saved");
    ...
    <div className="fixed inset-0 flex flex-col" style={{ background: "#080810" }}>
      ...
      >
        ...
        <div className="flex-1">
          ...
          <p className="text-xs" style={{ color: "#555" }}>{config.batchId} · Fully automatic</p>
        </div>
        {saveStatus === "saving" && (
          <span className="text-xs font-medium px-2 py-1 rounded-full" style={{ background:"rgba(6,182,212,0.15)", color:"#06b6d4" }}>Saving…</span>
        )}
        {saveStatus === "saved" && (
          <span className="text-xs font-medium px-2 py-1 rounded-full" style={{ background:"rgba(34,197,94,0.15)", color:"#22c55e" }}>Saved ✓</span>
        )}
        {saveStatus === "error" && (
          <span className="text-xs font-medium px-2 py-1 rounded-full" style={{ background:"rgba(239,68,68,0.15)", color:"#ef4444" }} title={saveError ?? ""}>Save failed</span>
        )}
      </div>
      <div className="flex-1 relative">
        <InspectionView
          ...
          onNeedCalibration={() => router.push("/admin/calibrate")}
        />
      </div>
    </div>
  );
