...
function loadInferencejs(): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    if (existing) {
      ...
      existing.addEventListener("error", () => reject(new Error("inferencejs CDN load failed")));
      return;
    }
...
export function CameraView({ onCapture, onError }: Props) {
  ...
  // Load inferencejs + model eagerly once camera ready
  useEffect(() => {
    ...
    async function loadModel() {
      ...
      try {
        ...
        setStatusMsg("Model load failed — tap capture to retry");
      } finally {
  ...
  const handleCapture = useCallback(async () => {
    ...
    try {
      ...
      console.log("[inferencejs]", predictions.length, "predictions", predictions);
    } catch (e) {
      console.error("[inferencejs infer]", e);
    ...
    if (predictions.length === 0) {
      setIsCapturing(false);
      onError("No shoes detected. Ensure both shoes are clearly visible in frame.");
      return;
    }

    // Convert predictions to BBox (Roboflow returns center x,y,w,h)
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    function predToBBox(p: any): BBox {
      // inferencejs returns {x, y, width, height} as center-based
      const x = p.bbox?.x ?? p.x ?? 0;
      const y = p.bbox?.y ?? p.y ?? 0;
      const w = p.bbox?.width ?? p.width ?? 0;
      const h = p.bbox?.height ?? p.height ?? 0;
      return {
        minX: Math.round(x - w / 2),
        maxX: Math.round(x + w / 2),
        minY: Math.round(y - h / 2),
        maxY: Math.round(y + h / 2),
      };
    }

    // Filter small detections, sort left→right, take best 2
    const filtered = predictions
      .filter(p => {
        const w = p.bbox?.width ?? p.width ?? 0;
        const h = p.bbox?.height ?? p.height ?? 0;
        return w > vw * 0.05 && h > vh * 0.08;
      })
      .sort((a, b) => {
        const ax = a.bbox?.x ?? a.x ?? 0;
        const bx = b.bbox?.x ?? b.x ?? 0;
        return ax - bx;
      });
    ...
    // Draw polygon if segmentation points available, else rect
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    <div className="relative w-full h-full bg-black overflow-hidden">
      ...
      {isReady && !isCapturing && (
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-10 flex items-center gap-2">
          <div className="px-4 py-1.5 rounded-full text-xs font-semibold text-center whitespace-nowrap flex items-center gap-2"
            style={{ background: "rgba(0,0,0,0.75)", border: `1px solid ${CYAN}40`, color: "#ccc" }}>
            <span className="w-2 h-2 rounded-full inline-block"
              style={{ background: modelReady ? GREEN : modelLoading ? "#f59e0b" : "#666", boxShadow: modelReady ? `0 0 6px ${GREEN}` : "none" }} />
            {modelReady ? "AI Ready" : modelLoading ? "Loading model..." : "Side view · both shoes in frame"}
          </div>
        </div>
      )}
